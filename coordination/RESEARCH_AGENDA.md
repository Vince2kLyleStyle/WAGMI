...
30. CONCORDANCE ENGINE (owner, 2026-07-02 — "all-encompassing quant"): the standing cross-validation of EVERY knowledge source against every other: live-era realized (WR/expectancy per regime/setup/side) vs replay-campaign predictions vs backtest variants (exit geometry, sizing ladders) vs thesis grades vs mechanical baselines (ADX/ATR nowcast, depth signals) vs counterfactual would-haves. One recurring CONCORDANCE.md table: predicted vs realized per cell, divergence flags (>1 sigma = investigate: wiring fault or edge decay). Runs weekly with the meta-audit + on every 10 new clean closes. Concordance across sources = the ONLY license to advance the sizing ramp; divergence = automatic pause + investigation. This is the all-encompassing-quant identity operationalized.
