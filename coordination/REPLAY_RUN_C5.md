# REPLAY_RUN_C5 — full-system historical replay
Generated 2026-07-02 22:25 UTC by tools/replay_harness.py (THE_STANDARD v1.3 compliant reporting)
...
## Window & config
- Window: 2026-02-01 -> 2026-02-08 (walk); fetch depth 11d (extra = indicator warmup)
- Symbols: BTC,ETH,SOL
...
- Total LLM calls: 180 (cap 180; cap reached: True)
- Journal entries: 44 | failures: 0 | pre-filter skips: 342
- Entry-event filter: 103 qualifying events | starved by caps: 41 | cooldown-suppressed: 37 | per-symbol calls: {"BTC": 62, "ETH": 60, "SOL": 58}
- Wall time: 120 min
- SCALING MATH: ~0.0 closes per 60 LLM calls at this signal density (0 closes / 180 calls)

## Isolation proof
- Sandbox: bot/data/replay/C5/sandbox (code copy + empty data tree; runner refuses to start outside a marked sandbox)
...
- CHANGED PATHS (expected: live-bot churn only — the replay process has no handle to these by construction; verify none are backtest/replay artifacts):
    ...
    - data/manual/conviction_sizing.jsonl
    - data/manual/pa_sim_status.json
    - data/manual/pa_sim_trades.jsonl
...
Artifacts: bot/data/replay/C5/replay_trades.csv, run.log, isolation_report.json, sandbox/replay_out/*