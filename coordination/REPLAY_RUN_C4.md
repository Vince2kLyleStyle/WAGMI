# REPLAY_RUN_C4 — full-system historical replay
Generated 2026-07-02 20:24 UTC by tools/replay_harness.py (THE_STANDARD v1.3 compliant reporting)
...
## Window & config
- Window: 2025-06-07 -> 2025-06-14 (walk); fetch depth 11d (extra = indicator warmup)
- Symbols: BTC,ETH,SOL
...
- Net PnL (after fees+funding): $+0.44 on $500 equity | fees paid $0.05
- Final equity: $500.46
- Per-side: {"LONG": {"n": 2, "wins": 2, "wr": 100.0, "pnl": 0.44}}
- Per-regime: {"trending_bull": {"n": 2, "wins": 2, "wr": 100.0, "pnl": 0.44}}

## LLM usage (honest accounting)
- Total LLM calls: 182 (cap 180; cap reached: True)
- Journal entries: 47 | failures: 0 | pre-filter skips: 404
- Entry-event filter: 102 qualifying events | starved by caps: 14 | cooldown-suppressed: 46 | per-symbol calls: {"BTC": 62, "ETH": 62, "SOL": 58}
- Wall time: 136 min
- SCALING MATH: ~0.7 closes per 60 LLM calls at this signal density (2 closes / 182 calls)

## Isolation proof
- Sandbox: bot/data/replay/C4/sandbox (code copy + empty data tree; runner refuses to start outside a marked sandbox)
- Production data diff (bot/data, bot/ml_data, bot/backtest_ml_data, bot/trades.csv; before vs after): 75 paths changed
- CHANGED PATHS (expected: live-bot churn only — the replay process has no handle to these by construction; verify none are backtest/replay artifacts):
    ...
    - data/llm/self_analyst_last_run.json
...
Artifacts: bot/data/replay/C4/replay_trades.csv, run.log, isolation_report.json, sandbox/replay_out/*